...
def run_seqnetmodel(config):
    ...
    train_feature1_np = np.array(feature_data[1])
    train_feature1_label_np = np.array(feature_label[1])
    train_feature2_np = np.array(feature_data[2])
    train_feature2_label_np = np.array(feature_label[2])
    ...
    for epoch_num in range(config.n_iter):
        ...
        for batchID in range(num_batches):
            ...
            batch_feature1 = train_feature1_np[batch_record_index]
            batch_feature1_label = train_feature1_label_np[batch_record_index]
            batch_feature2 = train_feature2_np[batch_record_index]
            batch_feature2_label = train_feature2_label_np[batch_record_index]
            ...
            batch_feature1 = torch.from_numpy(batch_feature1).type(torch.LongTensor).to(device)
            batch_feature1_label = torch.from_numpy(batch_feature1_label).type(torch.LongTensor).to(device)
            batch_feature1_label = F.one_hot(batch_feature1_label, num_classes=feature_num[1]).squeeze(1)
            batch_feature2 = torch.from_numpy(batch_feature2).type(torch.LongTensor).to(device)
            batch_feature2_label = torch.from_numpy(batch_feature2_label).type(torch.LongTensor).to(device)
            batch_feature2_label = F.one_hot(batch_feature2_label, num_classes=feature_num[2]).squeeze(1)
            prediction_score, prediction_score0, prediction_score1, prediction_score2\
                = seqnet_model(batch_data, batch_feature0, batch_feature1, batch_feature2)
            ...
            if config.model == "gru":
                ...
                if len(batch_feature1_label.size()) == 3:
                    loss_1 = loss_fun_mul(prediction_score1, ((torch.sum(batch_feature1_label,dim=1)+2001)/2002).float())
                    loss_1 = torch.mean(torch.sum(loss_1))
                else:
                    loss_1 = loss_fun_mul(prediction_score1, batch_feature1_label.float())
                    loss_1 = torch.mean(torch.sum(loss_1))

                if len(batch_feature2_label.size()) == 3:
                    loss_2 = loss_fun_mul(prediction_score2, ((torch.sum(batch_feature2_label,dim=1)+2001)/2002).float())
                    loss_2 = torch.mean(torch.sum(loss_2))
                else:
                    loss_2 = loss_fun_mul(prediction_score2, batch_feature2_label.float())
                    loss_2 = torch.mean(torch.sum(loss_2))
                ...
                if len(batch_feature1_label.size()) == 4:
                    loss_1 = loss_fun_mul(prediction_score1,
                                          ((torch.sum(batch_feature1_label, dim=2) + 2001) / 2002).float())
                    loss_1 = torch.mean(torch.sum(loss_1))
                else:
                    loss_1 = loss_fun_mul(prediction_score1, batch_feature1_label.float())
                    loss_1 = torch.mean(torch.sum(loss_1))

                if len(batch_feature2_label.size()) == 4:
                    loss_2 = loss_fun_mul(prediction_score2,
                                          ((torch.sum(batch_feature2_label, dim=2) + 2001) / 2002).float())
                    loss_2 = torch.mean(torch.sum(loss_2))
                else:
                    loss_2 = loss_fun_mul(prediction_score2, batch_feature2_label.float())
                    loss_2 = torch.mean(torch.sum(loss_2))
                ...
                loss = loss_ + (loss_0+ loss_1 + loss_2)*100
        ...
        if (epoch_num + 1) % 1 == 0:
            ...
            if ndcg[1] > max_ans:
                max_ans = ndcg[1]
                output_str = "Epoch %d  loss_=%.4f, loss_0=%.4f, loss_1=%.4f, loss_2=%.4f" % \
                             (epoch_num + 1, loss_, loss_0, loss_1, loss_2)
                logger.info(output_str)
                #logger.info('p: '+', '.join(str(e) for e in precision))
                logger.info('r: ' + ', '.join(str(e) for e in recall))
                logger.info('MRR: '+', '.join(str(e) for e in MRR))
...
def seqnetmodel_evaluation(seq_model, data_dic, ttype="val"):
    ...
    for batchID in range(num_batches):
        ...
        batch_feature1 = train_feature1_np[start:end]
        batch_feature2 = train_feature2_np[start:end]

        batch_feature0 = torch.from_numpy(batch_feature0).type(torch.LongTensor).to(device)
        batch_feature1 = torch.from_numpy(batch_feature1).type(torch.LongTensor).to(device)
        batch_feature2 = torch.from_numpy(batch_feature2).type(torch.LongTensor).to(device)

        rating_pred, _,_,_ = seq_model(data_sequences, batch_feature0, batch_feature1, batch_feature2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_data', type=str, default="ML100K")
